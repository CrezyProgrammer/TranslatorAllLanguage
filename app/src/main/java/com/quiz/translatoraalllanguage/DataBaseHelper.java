...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public void insertWord(Word item) {
        if (!isInDataBase(item)) {
            Log.i("123321", "34: "+item.getWord()+" "+item.getTranslation());
    ...
    public String[] getLanguages(String word, String sourceLanguage, String targetLanguage) {
        ...
        languages[0] = cursor.getString(cursor.getColumnIndex("sourcePosition"));
        languages[1] = cursor.getString(cursor.getColumnIndex("targetPosition"));
    ...
    public ArrayList<Word> getAllWords() {
        ...
        while (!cursor.isAfterLast()) {
            Word item = new Word(cursor.getString(cursor.getColumnIndex("word")),
                    ...
                    cursor.getString(cursor.getColumnIndex("sourcePosition")),
                    cursor.getString(cursor.getColumnIndex("targetPosition")));
            if (!item.isEmpty()) {
                arrayList.add(item);
            }
            cursor.moveToNext();
